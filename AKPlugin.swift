...
class AKPlugin: NSObject, Plugin {
    ...
    private var hidDebugEventCounter = 0
    private let hidDebugMaxEvents = 180
    private lazy var hidDebugLogPath = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("pc-hid.log").path
    ...
    func setupHIDControllerInput(onConnected: @escaping () -> Void,
        ...
        hidDebugLog(String(format: "IOHIDManager configured with %d match profiles", matching.count))
    ...
    private func handleHIDDeviceConnected(_ device: IOHIDDevice) {
        if !isSupportedControllerDevice(device) {
            let product = hidDevicePropertyString(device, key: kIOHIDProductKey as CFString) ?? "unknown"
            ...
            hidDebugLog(String(format: "ignoring non-controller HID device: product=%@ usagePage=0x%X usage=0x%X vendor=0x%X product=0x%X",
                               product, usagePage, usage, vendorID, productID))
            return
        }
        ...
        if hidPrimaryDevice == nil {
            ...
            let product = hidDevicePropertyString(device, key: kIOHIDProductKey as CFString) ?? "unknown"
            let transport = hidDevicePropertyString(device, key: kIOHIDTransportKey as CFString) ?? "unknown"
            let usagePage = hidDevicePropertyInt(device, key: kIOHIDPrimaryUsagePageKey as CFString) ?? -1
            let usage = hidDevicePropertyInt(device, key: kIOHIDPrimaryUsageKey as CFString) ?? -1
            let vendorID = hidDevicePropertyInt(device, key: kIOHIDVendorIDKey as CFString) ?? -1
            let productID = hidDevicePropertyInt(device, key: kIOHIDProductIDKey as CFString) ?? -1
            hidDebugLog(String(format: "device connected: product=%@ transport=%@ usagePage=0x%X usage=0x%X vendor=0x%X product=0x%X",
                               product, transport, usagePage, usage, vendorID, productID))
            hidOnConnected?()
        }
    }
    ...
    private func handleHIDDeviceRemoved(_ device: IOHIDDevice) {
        ...
        hidDebugLog("primary HID device removed")
        ...
        if let manager = hidManager,
           ...
           let another = devices.first(where: { isSupportedControllerDevice($0) }) {
            ...
            let product = hidDevicePropertyString(another, key: kIOHIDProductKey as CFString) ?? "unknown"
            hidDebugLog(String(format: "switched to fallback HID device: %@", product))
            hidOnConnected?()
        }
    }
    ...
    private func handleHIDInputValue(_ value: IOHIDValue) {
        ...
        hidDebugEventCounter += 1
        let shouldLogEvent = hidDebugEventCounter <= hidDebugMaxEvents

        switch usagePage {
        case 0x09: // Button page
            if shouldLogEvent {
                hidDebugLog(String(format: "event #%d button usage=0x%X raw=%ld pressed=%@",
                                   hidDebugEventCounter, usage, rawValue, rawValue != 0 ? "true" : "false"))
            }
            hidOnButton?(usage, rawValue != 0)
        case 0x01: // Generic Desktop page
            if usage == 0x39 {
                if shouldLogEvent {
                    hidDebugLog(String(format: "event #%d hat raw=%ld", hidDebugEventCounter, rawValue))
                }
            ...
            if [0x30, 0x31, 0x32, 0x33, 0x34, 0x35].contains(usage) {
                ...
                if shouldLogEvent {
                    hidDebugLog(String(format: "event #%d axis usage=0x%X raw=%ld norm=%.4f",
                                       hidDebugEventCounter, usage, rawValue, normalized))
                }
                hidOnAxis?(usage, normalized)
            } else if shouldLogEvent {
                hidDebugLog(String(format: "event #%d generic usage=0x%X raw=%ld (ignored)",
                                   hidDebugEventCounter, usage, rawValue))
            }
        case 0x0C:
            if shouldLogEvent {
                hidDebugLog(String(format: "event #%d consumer usage=0x%X raw=%ld",
                                   hidDebugEventCounter, usage, rawValue))
            }
        default:
            if shouldLogEvent {
                hidDebugLog(String(format: "event #%d usagePage=0x%X usage=0x%X raw=%ld (unhandled)",
                                   hidDebugEventCounter, usagePage, usage, rawValue))
            }
            break
        }
    }
    ...
    private func hidDebugLog(_ message: String) {
        let line = "[PC-HID] \(message)"
        NSLog("%@", line)
        let url = URL(fileURLWithPath: hidDebugLogPath)
        if !FileManager.default.fileExists(atPath: hidDebugLogPath) {
            FileManager.default.createFile(atPath: hidDebugLogPath, contents: nil)
        }
